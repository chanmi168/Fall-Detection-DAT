...
# args = parser.parse_args(['--input_folder', '../../data_mic/stage2/modeloutput_18hz_5fold_UPFall_UMAFall_cross-config_reversed_earlystop_repeat',
...
for task_name, dict_task in dict_task_all.items():
    ...
    for key in dict_task.keys():
        ...
        print(dann_mean, fig_dir)
#         if dann_mean_optimal > dann_mean: # find the lowest loss
        if dann_mean_optimal < dann_mean: # find the highest F1
            dann_mean_optimal = dann_mean
            key_optimal = key
            print(dann_mean_optimal, key_optimal)
...
#         print('arr_F1:', arr_F1.mean())
#         print('test_F1:', test_F1.mean())
#         print('test_F1-arr_F1:', test_F1-arr_F1)
...
def F1_checker_mean(dict_df):
    for column_name in dict_df['sensitivity'].columns.values:
        for row_name in dict_df['sensitivity'].index.values:
            ...
            print(float_F1, test_F1)
            if np.abs(float_F1-test_F1) > 0.0001:
...
for task_name, dict_task in dict_task_all.items():
    ...
    for key in dict_task.keys():
        ...
        for i, metric_name in enumerate(metric_names):
            ...
            dict_df[metric_name].loc['source', key] = '{:.4f}±{:.4f}'.format(np.mean(source_dpt),np.std(source_dpt))
            dict_df[metric_name].loc['DANN', key] = '{:.4f}±{:.4f}'.format(np.mean(dann_dpt),np.std(dann_dpt))
            dict_df[metric_name].loc['target', key] = '{:.4f}±{:.4f}'.format(np.mean(target_dpt),np.std(target_dpt))
